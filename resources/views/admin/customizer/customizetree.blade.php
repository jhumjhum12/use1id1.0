...
<?php if($layout != 'Download Multiple Tables')  { ?>
      <button class="tablinks changeLayout <?php if($layout == 'Customizing(Inline)')  { ?>layoutfour <?php } ?>"  data-layout="{{$layout}}">{{$layout}}</button>
	
   <?php } else { ?>
	<button class="" OnClick="shownew(this.id);" data-layout="Customizing(Inline)" class="" id="last_1" >{{$layout}}</button>
   <?php } ?>
 @endforeach
</div>
